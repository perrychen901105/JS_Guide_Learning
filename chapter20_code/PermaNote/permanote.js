...
window.onload = function() {
    // 第一次载入时，初始化本地存储
    if (this.localStorage.note == null) {
        localStorage.note = "";
    }
    ...
    editor.value = localStorage.note;   // 初始化编译器，将保存的笔记数据填充为其内容
...
window.onbeforeunload = function() {
    if (localStorage.lastModified > this.localStorage.lastSaved) save();
};
...
window.applicationCache.onnoupdate = function() {
    status("You are running the lateest version of the application.");
}
